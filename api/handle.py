...
def handle_message(update_data):
    ...
    if update.type == "command":
        response_text = excute_command(
            update.from_id, update.text, update.from_type, update.chat_id
        )
        if response_text != "":
        ...
        if update.is_group:
            send_message(
                update.chat_id, f"{group_no_permission_info}\nID:`{update.chat_id}`"
            )
        ...
        extra_text = (
            f"\n\n{prompt_new_info}"
            if chat.history_length >= prompt_new_threshold * 2
            else ""
        )
        ...
        if update.is_group:
            ...
            log = (
                f"@{update.user_name} id:`{update.from_id}`{send_unrecognized_content}"
            )
        send_log(log)
